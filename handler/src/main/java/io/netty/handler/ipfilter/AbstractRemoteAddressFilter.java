...
public abstract class AbstractRemoteAddressFilter<T extends SocketAddress> extends ChannelInboundHandlerAdapter {
    ...
    private boolean handleNewChannel(ChannelHandlerContext ctx) throws Exception {
        ...
        ctx.pipeline().remove(this); // 只判断一次

        if (accept(ctx, remoteAddress)) { // 判断是否接受这个地址
            channelAccepted(ctx, remoteAddress); // 当前已有 channelAccepted 实现返回都是()，所以什么都不做
        } else { // 当前已有 channelRejected 实际返回都是 null，所以执行"关闭"
